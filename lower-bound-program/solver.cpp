    ...
    for (int d = 0; d <= N; ++d) {
        for (int w = 0; w <= d; ++w) {
            ...
            // LTP row
            {
                ...
                int num_y = (d < N) ? (d + 1) : N;
                // -1 sentinels: y_scaled=-1 means LTP_BOT choice; x_scaled=-1 means N/A
                double y_scaled = -1.0, x_scaled = -1.0;

                if (ei >= 0 && ei < num_y) {
                    int y = (d == N) ? ei : (ei == 0 ? 0 : N - d + (ei - 1));
                    y_scaled = (double)y / (double)N;
                    int ltptop = g.ltptop_node(d, w, y);
                    if (ltptop >= 0) {
                        int min_ei = tau[ltptop];
                        if (min_ei >= 0) {
                            int x_min = std::max(y + 1, N - d);
                            int x = x_min + min_ei / 2;
                            x_scaled = (double)x / (double)N;
                        }
                    }
                }
                // ei == num_y means LTP_BOT was chosen; y_scaled stays -1
    ...
    const std::string&      filename)
{
    ...
    ofs << "d,w,y,x_value,dest_mode\n";
    ofs << std::fixed << std::setprecision(10);

    for (int d = 0; d <= N; ++d) {
        for (int w = 0; w <= d; ++w) {
            // Enumerate valid y values: y=0, then y in [N-d, N-1] (or [1,N-1] if d==N)
            auto emit = [&](int y) {
                int ltptop = g.ltptop_node(d, w, y);
                if (ltptop < 0) return;
                int min_ei = tau[ltptop];
                ...
                int x      = x_min + min_ei / 2;
                int mode   = min_ei % 2;
                ofs << (double)d / N << "," << (double)w / N << ","
                    << (double)y / N << "," << (double)x / N << ","
                    << mode << "\n";
            };

            emit(0);
            int y_start = (d < N) ? (N - d) : 1;
            for (int y = y_start; y <= N - 1; ++y)
                emit(y);
        }
    }

    ofs.close();
    std::cerr << "Saved LTP_TOP_MIN adversary policy CSV to " << filename << "\n";
}

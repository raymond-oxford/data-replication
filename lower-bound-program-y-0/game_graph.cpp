...
GameGraph::GameGraph(int N) : N_(N), p_(0), q_(1) {
    ...
    // 3. LTP_TOP_MIN nodes: d=0..N, w=0..d
    for (int d = 0; d <= N; ++d) {
        for (int w = 0; w <= d; ++w) {
            ltptop_arr_[ltptop_idx(d, w)] = id;
            NodeMeta m{};
            m.type = NodeType::LTP_TOP_MIN;
            m.d = d; m.w = w; m.y = 0; m.mode = 0;
            m.num_edges = (d < N) ? 2 * (d + 1) : 2 * N;
            meta_.push_back(m);
            ++id;
        }
    }
...
int GameGraph::ltptop_node(int d, int w) const           { return ltptop_lookup(d, w); }
...
int GameGraph::ltptop_lookup(int d, int w) const {
    if (d < 0 || d > N_ || w < 0 || w > d) return -1;
    return ltptop_arr_[ltptop_idx(d, w)];
}
...
EdgeInfo GameGraph::edge_base(int u, int ei) const {
    ...
    if (m.mode == 0) {
        ...
        // LTP mode: ei=0 -> ltptop[d,w], ei=1 -> LTP_BOT
        if (ei == 0)
            e.to = ltptop_lookup(m.d, m.w);
        else
            e.to = ltp_bot_id_;
    }
    assert(e.to != -1);
...
EdgeInfo GameGraph::edge_ltptop(int u, int ei) const {
    ...
    int x_min = (d == N_) ? 1 : 0;
    int x = x_min + ei / 2;
    int mode = ei % 2; // 0 or 1
    int d2 = N_ - d + x;
    int w2 = std::min(N_ - w, d2);
    EdgeInfo e{};
    e.to = base_lookup(d2, w2, mode);
    assert(e.to != -1);
    e.opt_num = N_ - d + w + x;
    e.alg_num = 2 * N_ - d + x;
    e.reward = p_ * e.opt_num - q_ * e.alg_num;
